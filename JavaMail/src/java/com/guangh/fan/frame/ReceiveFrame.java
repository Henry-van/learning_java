...
public class ReceiveFrame extends JInternalFrame implements MouseListener {
    ...
    private Object[] columnTitle = {"发件人" , "主题" , "发送时间", "附件"}; // 定义邮件Table的列标题
    ...
    private void initData() {
        ...
        // 但imap协议保存邮件到本地，存在问题（JavaMail BaseEncode64 Error）
//        receiveMail = new ReceiveMail("pop3", "inbox", false);
//        receiveMail.receive();
...
//        msgList = receiveMail.getMailList();
    ...
    private int initFromLocal() {
        ...
        curDir = f.getPath();
        File dir = new File(curDir + RECEIVE_PATH);
//        fileName = dir.list();
        ...
        for (int i = 0; i < messageCount; i++) {
            try {
                ...
                // 创建邮件对象
                FileInputStream fis = new FileInputStream(curDir + RECEIVE_PATH + file);
                    ...
                    try {
                        ...
                        itemdel.addActionListener( new ActionListener() {
                            public void actionPerformed(ActionEvent arg0) {
                                if (JOptionPane.showConfirmDialog(ReceiveFrame.this, "确定要删除该邮件吗？") == 0) {
                                    // 从本地删除邮件 Start ************************
                                    File delFile = new File(curDir + RECEIVE_PATH + fileName[row]);
...
//                                    }
                        ...
                        List<String> fileList = new ArrayList<>();
                        if ( mailUtil.isContainAttach(clickedMsg, fileList)) {
                            JMenuItem itemSave = new JMenuItem("保存附件");
                            itemSave.addActionListener( new ActionListener() {
                                public void actionPerformed(ActionEvent arg0) {
                                    if (JOptionPane.showConfirmDialog(ReceiveFrame.this, "确定要要保存附件吗？") == 0) { // 如果选择确定键
                                        try {
                                            mailUtil.saveAttachMent(clickedMsg,"." + ATTACH_PATH);
                                        } catch (Exception ex) {
                                            ex.printStackTrace();
                                        }
                                    }

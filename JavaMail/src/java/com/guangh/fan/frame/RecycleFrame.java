...
public class RecycleFrame extends JInternalFrame implements MouseListener {
    private JScrollPane scrollPane = null; // 正文编辑窗口
    private JTextPane sendCotent; // 临时HTML转换用面板
    ...
    private void initData() {
        ...
        // 服务器获取已发送邮件 Start ################################
//        receiveMail = new ReceiveMail("imap", "Trash", true);
//        receiveMail.receive();
...
//        msgList = receiveMail.getMailList();
    ...
    private int initFromLocal() {
        ...
        curDir = f.getPath();
        File dir = new File(curDir + DELETE_PATH);
        ...
        for (int i = 0; i < messageCount; i++) {
            try {
                ...
                // 创建邮件对象
                FileInputStream fis = new FileInputStream(curDir + DELETE_PATH + file);
    ...
    private void initUI() {
        ...
        this.setFrameIcon(EditorUtils.createIcon("delete.png"));
        // 初始化基本项
        ...
        mailTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getButton() == 1) {
                    ...
                    JMenuItem itemdel = new JMenuItem("彻底删除");
                    itemdel.addActionListener( new ActionListener() {
                        public void actionPerformed(ActionEvent arg0) {
                            if (JOptionPane.showConfirmDialog(RecycleFrame.this, "确定要彻底删除该邮件吗？") == 0) {
                                // 从本地删除邮件 Start ************************
                                File delFile = new File(curDir + DELETE_PATH + fileName[row]);
                                delFile.delete(); // 把本地邮件从delete目录下彻底删除
